...
void solve() {
	...
	if (cnt == n) {
		...
		while (cnt > 0) {
			...
			for (auto x : indr) {
				if (x + 1 < n && s[x + 1] == '?') s[x + 1] = 'B';
				if (x - 1 >= 0 && s[x - 1] == '?') s[x - 1] = 'B';

			}

			for (auto x : indb) {
				if (x + 1 < n && s[x + 1] == '?') s[x + 1] = 'R';
				if (x - 1 >= 0 && s[x - 1] == '?') s[x - 1] = 'R';

			}
			int cnt1 = 0;
			fo(i, 0, n) if (s[i] == '?') cnt1++;
			cnt = cnt1;
			debug(mp(indr, indb));
			debug(s);
		}
	...
	fo(i, 0, n) {
		if (i % 2 == 0 && s1[i] == '?') s1[i] = 'B';
		else if (i % 2 != 0 && s1[i] == '?') s1[i] = 'R';
	}

	fo(i, 0, n) {
		if (i % 2 == 0 && s2[i] == '?') s2[i] = 'R';
		else if (i % 2 != 0 && s2[i] == '?') s2[i] = 'B';
	}
