...
void solve() {
	...
	//vector < vector<pair<int, int>> all(n, vector<pair<int, int>>(m));
	...
	//fo(i, 0, n * m) ma[alla[i].sc] = i + 1;

	int ind = 0;

	fo(i, 0, n) fo(j, 0, m) { ma[alla[ind].sc] = {i + 1, j + 1}; ind++;}

	vector<vector<bool>> iso(n, vector<bool>(m, 0));
