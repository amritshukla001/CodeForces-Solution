...
void solve() {
	...
	for (int i = 1; i <= n - 2; i += 2) oddv.pb(i);
	for (int i = 2; i <= n - 1; i += 2) evenv.pb(i);
	...
	while (a != sortv) {
		if (si % 2 != 0) {
			fo(i, 0, oddv.size()) {
				if (a[oddv[i] - 1] > a[oddv[i]]) swap(a[oddv[i] - 1], a[oddv[i]]);
			}
		}
		else {
			fo(i, 0, evenv.size()) {
				if (a[evenv[i] - 1] > a[evenv[i]]) swap(a[evenv[i] - 1], a[evenv[i]]);
			}
		}
		si++;
		debug(a);
