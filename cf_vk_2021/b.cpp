...
void solve() {
	...
	//Case 1: When alice served first
	if ((a + b) % 2 == 1) as = mid + 1, bs = mid;
	else as = bs = mid;

	fo1(i, 0, as) {
		int pos = a - (as - i);
		if (pos >= 0 and pos <= bs) posk.insert(i + pos);
	}

	//Case 2: When borys served firs
	if ((a + b) % 2 == 1) as = mid, bs = mid + 1;
	else as = bs = mid;
