...
void cal_sieve() {
	...
	for (int i = 2; i * i < size; i++) if (spf[i] == i)
			for (int j = i * i; j < size; j += i) if (spf[j] == j)
					spf[j] = i;
}
...
void solve() {
	...
	fo(i, 0, n) {
		debug(cnt[grp[a[i]]]);
		if (check[grp[a[i]]] == 1) {
			tot -= (cnt[grp[a[i]]] * (cnt[grp[a[i]]] - 1)) / 2;
			check[grp[a[i]]] = 0;
		}
	}
...
int32_t main() {
	...
	cal_sieve();
	fo(i, 1, size) {
		map<int, int> res = calc(i);
		for (auto x : res) {
			if (x.sc % 2 != 0) grp[i] *= x.fr;
		}
	}
	//debug(grp);
	testcase solve();
